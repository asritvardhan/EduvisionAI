...
class QLearningPersonalizer:
    ...
    def _calculate_reward(self, engagement_change, performance_change, action_taken):
        ...
        if engagement_score < 2:
            if action_taken in [0, 1, 5, 6, 7, 8]:
                reward += 0.5
            else:
                reward -= 0.5
        else:
            if action_taken in [3, 4]:
                reward += 0.5
            else:
                reward -= 0.5
        return float(reward)
    ...
    def process_engagement_data(self, user_id, engagement_score, vocal_score, fused_score,
        ...
        if quiz_score is not None:
            ...
            performance_score = float(fused_score * 0.7 + 0.3)
        ...
        try:
            user_performance_db.update_one({"user_id": user_id}, update_data_converted)
        except Exception as e:
            print(f"❌ Error updating user performance: {e}")
            try:
                update_data_bson = json.loads(json_util.dumps(update_data_converted))
                user_performance_db.update_one({"user_id": user_id}, update_data_bson)
        ...
        try:
            q_learning_db.update_one({"user_id": user_id}, {"$set": q_learning_update}, upsert=True)
        except Exception as e:
            print(f"❌ Error saving Q-learning state: {e}")
            try:
                q_learning_update_converted = json.loads(json_util.dumps(q_learning_update))
                q_learning_db.update_one({"user_id": user_id}, {"$set": q_learning_update_converted}, upsert=True)
            except Exception as e2:
                print(f"❌ BSON conversion also failed: {e2}")
...
def simplify_with_gemini(content, query, difficulty="normal", user_id=None):
    ...
    difficulty_guidelines = {
        "beginner": {"description": "Use very simple language, short sentences, and define any technical terms.", "target_fkgl": 5.0, "paragraphs": "8-10 clear, concise paragraphs", "sentence_length": "8-12 words average"},
        "easy": {"description": "Use simple language with basic explanations of technical terms.", "target_fkgl": 6.0, "paragraphs": "6-8 well-structured paragraphs", "sentence_length": "12-15 words average"},
        "normal": {"description": "Use clear, educational language with balanced detail.", "target_fkgl": 8.0, "paragraphs": "5-7 informative paragraphs", "sentence_length": "15-18 words average"},
        "intermediate": {"description": "Use straightforward language with some technical terms explained.", "target_fkgl": 10.0, "paragraphs": "4-6 detailed paragraphs", "sentence_length": "18-22 words average"},
        "advanced": {"description": "Use more technical terms but still make it accessible.", "target_fkgl": 12.0, "paragraphs": "3-5 comprehensive paragraphs", "sentence_length": "20-25 words average"}
    }
...
"""
    ...
    try:
        response = gemini_model.generate_content(
            prompt,
            generation_config={"temperature": 0.5, "top_p": 0.95, "max_output_tokens": 1500, "top_k": 40},
        )
        text, _ = extract_text_from_response(response)
        if text:
            cleaned_text = clean_text(text)
            final_fkgl = readability_analyzer.calculate_flesch_kincaid(cleaned_text)
            _, readability_metrics = readability_analyzer.adjust_content_for_readability(cleaned_text, guideline['target_fkgl'])
            return {
            ...
            return {"content": "(Simplification failed)", "original_fkgl": float(original_fkgl), "final_fkgl": 0.0, "readability_improvement": 0.0, "is_accessible": False, "difficulty_level": difficulty_level, "target_fkgl": float(guideline['target_fkgl'])}
    except Exception as e:
        print(f"⚠️ Gemini simplification error: {e}")
        return {"content": f"⚠️ Unable to simplify with AI. Here's the original content:\n\n{content[:800]}", "original_fkgl": readability_analyzer.calculate_flesch_kincaid(content[:800]), "final_fkgl": 0.0, "readability_improvement": 0.0, "is_accessible": False, "difficulty_level": difficulty_level, "target_fkgl": float(guideline['target_fkgl'])}
...
def api_qlearning_personalize():
    try:
        ...
        adjustments = q_learner.process_engagement_data(
            user_id=user_id, engagement_score=engagement_score, vocal_score=vocal_score,
            fused_score=fused_score, engagement_state=engagement_state, quiz_score=quiz_score
        )
...
def api_get_user_profile():
    try:
        ...
        user_record = user_performance_db.find_one({"user_id": user_id})
        if not user_record:
            return jsonify({"success": True, "profile_exists": False, "message": "No profile found for this user"})
        ...
        avg_recent_engagement = np.mean([safe_float(e.get("fused_score", 0)) for e in engagement_history[-5:]]) if engagement_history else 0.0
        avg_recent_performance = np.mean([safe_float(p.get("score", 0)) for p in performance_history[-5:]]) if performance_history else 0.0
        ...
        profile = {
            ...
            "difficulty_level": q_learner.difficulty_levels.get(int(user_record.get("current_difficulty", 2)), "normal"),
        ...
        return jsonify({"success": True, "profile_exists": True, "profile": convert_to_python_types(profile)})
    except Exception as e:
...
def api_generate_tts():
    try:
        ...
        return jsonify({"success": True, "tts_url": None, "use_browser_tts": True, "message": "Use browser TTS for instant playback"})
    except Exception as e:
        print(f"❌ TTS request error: {e}")
        return jsonify({"success": False, "error": str(e), "use_browser_tts": True, "message": "Falling back to browser TTS"}), 500
        ...
        try:
            ...
            for i, q in enumerate(quiz):
                if not isinstance(q, dict):
                    quiz[i] = {"question": f"Question {i+1}", "options": ["A", "B", "C", "D"], "answer": "A", "difficulty": difficulty}
                else:
            ...
            return jsonify({"success": True, "quiz": quiz, "difficulty": difficulty, "num_questions": len(quiz), "user_id": user_id})
        except json.JSONDecodeError as e:
            print(f"❌ JSON decode error: {e}")
            default_quiz = [{"question": f"What is the main topic discussed about {topic}?", "options": ["Its basic definition", "Its historical background", "Its practical applications", "All of the above"], "answer": "A", "difficulty": difficulty}]
            return jsonify({"success": True, "quiz": default_quiz, "difficulty": difficulty, "num_questions": 1, "user_id": user_id})
    except Exception as e:
...
def api_submit_quiz_results():
    try:
        data = request.get_json()
        user_id = data.get("user_id")
        quiz_score = float(data.get("score", 0))
        ...
        quiz_record = {
            ...
            "performance_category": "excellent" if quiz_score >= 0.8 else "good" if quiz_score >= 0.6 else "fair" if quiz_score >= 0.4 else "poor"
        }
        user_performance_db.update_one(
            {"user_id": user_id},
            {"$push": {"quiz_history": convert_to_python_types(quiz_record)}, "$set": {"last_quiz_score": float(quiz_score)}},
            upsert=True
        )
        recent_engagement = user_performance_db.find_one({"user_id": user_id}, {"engagement_history": {"$slice": -1}})
        if recent_engagement and "engagement_history" in recent_engagement:
            ...
            return jsonify({"success": True, "quiz_recorded": True, "q_learning_updated": True, "adjustments": adjustments, "new_difficulty": q_learner.get_difficulty_level(), "performance_feedback": quiz_record["performance_category"]})
        else:
            return jsonify({"success": True, "quiz_recorded": True, "q_learning_updated": False, "message": "Quiz results recorded. Q-learning update requires engagement data."})
    except Exception as e:
...
# *** FIX: PatchedInputLayer handles legacy batch_shape argument ***
# ------------------------- #
...
class PatchedInputLayer(tf.keras.layers.InputLayer):
    """Fixes TensorFlow deserialization error for models saved with older Keras versions.
    Translates legacy 'batch_shape' argument to the new 'shape' argument."""
...
try:
    ...
    # ✅ Both models now use PatchedInputLayer to handle legacy batch_shape
    ...
    video_model = load_model(
        video_model_path,
        custom_objects={"Attention": Attention, 'InputLayer': PatchedInputLayer},
        compile=False
    )
...
def api_engagement():
    ...
    try:
        ...
        if res_feats.shape[0] < 16:
            padding = np.zeros((16 - res_feats.shape[0], res_feats.shape[1]))
        ...
        try:
            q_learner.load_user_state(user_id)
            adjustments = q_learner.process_engagement_data(
                user_id=user_id, engagement_score=video_score, vocal_score=vocal_score,
                fused_score=fused, engagement_state=state
            )
            set_difficulty_level = q_learner.get_difficulty_level()
        except Exception as q_error:
            print(f"⚠️ Q-learning error (non-fatal): {q_error}")
            adjustments = {"difficulty_change": 0, "modality_change": None, "additional_support": False, "simplify_language": False, "action_taken": -1, "difficulty_level": "normal", "reason": "Q-learning initialization in progress"}
            set_difficulty_level = "normal"
        ...
        try:
            return jsonify({
                ...
                "video_probs": [],
                "audio_probs": [],
                "personalization": {"adjustments": {}, "current_difficulty": "normal", "reason": "Q-learning initialization failed, using default settings"},
